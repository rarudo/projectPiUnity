...
public class SumRecordsInspector : MonoBehaviour
{
	...
	IEnumerator FuncCoroutine() {
		while(true){
			...
			Dictionary<string, object> dic = Json.Deserialize(www.text) as Dictionary<string, object>;
			{
				...
				print (c_data);
				print (c_ip_infomation);
				data_.Add(c_data);
				ip_infomation_.Add(c_ip_infomation);
			}
	...
	void OnGUI()
	{
		...
		if (data_.Count > 0) {
			...
			for (var i = 0; i < data_.Count; ++i) {
				...
				Drawing.DrawLine(previousPos, currentPos, Color.red, 1f, true);
